...
def create_left_prompt [] {
    ...
    let host = ([$"(whoami)", '@', $"(hostname)"] | str join)

    let path_color = (if (is-admin) { ansi green_bold } else { ansi blue_bold })
    let separator_color = (if (is-admin) { ansi light_green_bold } else { ansi light_blue_bold })
    let path_segment = $"($path_color)($dir)(ansi reset)"

    [$"(ansi wb)➜ ", $"(ansi rb)($host)", $'(ansi white):(ansi reset)', ($path_segment | str replace --all (char path_sep) $"($separator_color)(char path_sep)($path_color)")] | str join
}
...
def prompt [] {
    if (is-admin) {$"(ansi wb)%(ansi reset) "} else {$"(ansi wb)$(ansi reset) "}
}

def trans_prompt [] {
    let host = ([$"(whoami)", '@', $"(hostname)"] | str join)
    [$"(ansi wb)➜ ", $"(ansi rb)($host)"] | str join
} 
...
$env.TRANSIENT_PROMPT_COMMAND = {|| trans_prompt }
